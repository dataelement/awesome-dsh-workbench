...
| 名称 | 作者填写 name，属于展示文案 | 作者填写 name，作为市场列表页的展示名称，与仓库身份独立 |
...
| 截图 | 作者仓库 screenshots.json，保留旧集中表回退 | 按本项目要求与条目同一 YAML，源仓库 HTTPS 图片地址、数量及顺序由作者决定 |
...
字段分工固定下来：YAML 表达名称、分类、双语文案、截图和可选包选择；包及仓库提供运行信息；生成 JSON 提供客户端读取契约；人工审核证据留在 PR。调整这些边界需要同时修改 Schema、示例、校验器、消费者说明和回归测试，不能只删增字段。
