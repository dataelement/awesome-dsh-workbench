...
import { ROOT, generateCatalog } from './catalog-lib.mjs'
...
export async function validatePublishedCatalog(catalog) {
  ...
  for (const item of catalog.workbenches) {
    ...
    for (const image of item.screenshots) {
      if (image.width * image.height > 16 * 1024 * 1024 || !image.url.startsWith(`https://raw.githubusercontent.com/${item.owner}/${item.repository}/${item.sourceCommit}/`)) throw new Error('截图版本或像素范围无效')
    }
  }
  return catalog
}
