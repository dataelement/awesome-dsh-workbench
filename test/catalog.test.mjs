...
test('minimal example is the production protocol and derives identity from the repository', async () => {
...
test('rejects fields generated by the catalog or review process', async () => {
  const validate = await createValidator()
  const { entry } = await readEntry(fixture, validate)
  for (const key of ['id', 'schemaVersion', 'version', 'source', 'author', 'verification', 'requirements', 'npm', 'release']) {
